...
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Open WebUI (v10-studio) - Port 3000                      │  │
│  │ Interface Chat IA + RAG intégré                          │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ n8n (v10-n8n) - Port 5678 - v1.120.0                     │  │
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Apache Superset (v10-bi) - Port 8088                     │  │
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Prometheus (v10-prometheus) - Port 9090                  │  │
│  │ Time Series Database - Métriques 8 services              │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Grafana (v10-grafana) - Port 3001                        │  │
│  │ Dashboards Monitoring - 2 dashboards préconfiguré       │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ AlertManager (v10-alertmanager) - Port 9093              │  │
│  │ Gestion Alertes - Email/Slack Notifications              │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ Node Exporter (v10-node-exporter) - Port 9100            │  │
│  │ Métriques Système - CPU, RAM, Disk, Network             │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ cAdvisor (v10-cadvisor) - Port 8080                      │  │
...
**Routing Pattern**:
```
https://ia.oceanphenix.fr/          → Hub Frontend (v10-frontend:8080)
https://ia.oceanphenix.fr/api       → FastAPI (v10-api:8000)
https://ia.oceanphenix.fr/studio    → Open WebUI (v10-studio:3000)
https://ia.oceanphenix.fr/grafana   → Grafana (v10-grafana:3001)
https://ia.oceanphenix.fr/s3        → MinIO (v10-minio:9000)
https://ia.oceanphenix.fr/n8n       → n8n (v10-n8n:5678)
https://ia.oceanphenix.fr/portainer → Portainer (v10-portainer:9443)
https://ia.oceanphenix.fr/bi        → Superset (v10-bi:8088)
```
...
ia.oceanphenix.fr {
    ...
    reverse_proxy /api/* v10-api:8000
}
```
...
DATABASES = {
    'prometheus': {
        'engine': 'prometheus',
        'uri': 'prometheus://v10-prometheus:9090'
    },
    'postgresql': {
        'engine': 'postgresql',
        'uri': 'postgresql://postgres@v10-db:5432/oceanphenix'
    }
}
...
Scrape Targets (8)   Exporters (2)   AlertManager
  ...
  ┌───┴────┬───────┬───┴───┬─────┐   ┌────▼─────┐
  ▼        ▼       ▼       ▼     ▼   │v10-alert │
Ollama  FastAPI Qdrant MinIO Caddy   │Port 9093 │
        n8n                           │          │
                  Node     cAdvisor   │• Email   │
                  Exporter           │• Slack   │
                                      └──────────┘
